...
pub fn build(b: *std.Build) void {
    ...
    // Add a single test executable for the `omega_rag` library module itself.
    ...
    // Add another test executable for all other test files.
    // This is the more modern approach than looping.
    const individual_tests = b.addTest(.{
        .root_source_file = b.path("tests/all_tests.zig"),
        .target = target,
        .optimize = optimize,
    });
    individual_tests.root_module.addImport("omega_rag", omega_rag_module);
